...
* [FrameworkController 训练服务](./FrameworkControllerMode.md)：支持使用在 Kubernetes 上使用 FrameworkController 运行。
...
* 为 OpenPAI, Kubeflow 和 FrameworkController 模式提供更好的日志支持。 
  * 改进后的日志架构能将尝试的 stdout/stderr 通过 HTTP POST 方式发送给 NNI 管理器。 NNI 管理器将尝试的 stdout/stderr 消息存储在本地日志文件中。
  * 在 WEB 界面上显示尝试日志的链接。
* 支持将最终结果显示为键值对。
...
* **nni.report_final_result(result)** API 对结果参数支持更多的数据类型。 可用类型：
