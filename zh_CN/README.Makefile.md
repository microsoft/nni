...
NNI 使用 [GNU Make](https://www.gnu.org/software/make/) 来生成和安装。

`Makefile` 提供标准的目标 `生成`、`安装` 和 `卸载`, 以及不同设置的安装目标：
...
运行 NNI 需要 Node.js 及其依赖库。 Node.js 依赖库 (包括 TypeScript) 可以通过 Yarn 来安装， Python 依赖库可以通过 setuptools 或者 PIP 来安装。
...
此外，`nnictl` 提供一个自动完成的脚本，会被安装在 `/usr/share/bash-completion/completions` 或 `~/.bash_completion.d` 目录下。
...
注意，这些变量不仅会影响安装路径，也会影响生成的 `nnictl` 脚本。 如果复制文件的路径和运行时的不一样（例如，创建发行版本包时），需要手工编辑 `nnictl` 脚本。
...
| `dev-install`      | 将 Python 和 Node.js 模块作为符号链接来安装，然后安装 scripts |
