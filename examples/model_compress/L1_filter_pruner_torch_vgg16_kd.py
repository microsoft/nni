...
def train(model, model_t, device, train_loader, optimizer):
    model.train()
    ALPHA = 0.5
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss_kd = F.cross_entropy(output, output_t)
        loss = F.cross_entropy(output, target) + loss_kd * ALPHA
        loss.backward()
        optimizer.step()
        if batch_idx % 100 == 0:
...
def main():
    ...
    for epoch in range(40):
        ...
        train(model, model_t, device, train_loader, optimizer_finetune)
