...
parser = argparse.ArgumentParser(description='quantization quick start')

parser.add_argument("--task_name", type=str, default='mnli', help="task name in the GLUE")
parser.add_argument("--finetune_lr", type=float, default=4e-5)
parser.add_argument("--quant_lr", type=float, default=1e-5)
parser.add_argument("--quant_method", type=str, default='lsq')
parser.add_argument("--quant_max_epochs", type=int, default=10)
...
def prepare_traced_trainer(model, args, load_best_model_at_end=False, is_trace=False):
    ...
    train_dataset, validation_datasets = prepare_datasets(args.task_name, tokenizer, '')
    ...
    if is_trace:
        training_args.learning_rate = args.quant_lr
        ...
        training_args.learning_rate = args.finetune_lr
...
def build_finetuning_model(args, state_dict_path: str, is_trace=False):
    model = build_model('bert-base-uncased', args.task_name)
    if Path(state_dict_path).exists():
        model.load_state_dict(torch.load(state_dict_path))
    else:
        trainer = prepare_traced_trainer(model, args, True, is_trace)
        trainer.train()
...
def fake_quantize(args):
    ...
    model = build_finetuning_model(args, f'./output/bert_finetuned/{args.task_name}.bin', is_trace=False)
    traced_trainer = prepare_traced_trainer(model, args, is_trace=True)
    evaluator = TransformersEvaluator(traced_trainer)
    if args.quant_method == 'lsq':
        quantizer = LsqQuantizer(model, config_list, evaluator)
        model, calibration_config = quantizer.compress(max_steps=None, max_epochs=args.quant_max_epochs)
    elif args.quant_method == 'qat':
        quantizer = QATQuantizer(model, config_list, evaluator, 1000)
        model, calibration_config = quantizer.compress(max_steps=None, max_epochs=args.quant_max_epochs)
    elif args.quant_method == 'ptq':
        ...
        raise ValueError(f"quantization method {args.quant_method} is not supported")
...
def evaluate(args):
    model = build_finetuning_model(args, f'./output/bert_finetuned/{args.task_name}.bin', is_trace=False)
    trainer = prepare_traced_trainer(model, args, is_trace=False)
