...
if [ -f "w_data.zip" ]; then
    ...
    wget -O w_data.zip https://github.com/fmsnew/nas-bench-nlp-release/tree/master/train_logs_wikitext-2/logs.zip?raw=true
fi
unzip w_data.zip
...
if [ -f "ptb_data.zip" ]; then
    ...
    wget -O ptb_data.zip https://github.com/fmsnew/nas-bench-nlp-release/tree/master/train_logs_single_run/logs.zip?raw=true
fi
unzip ptb_data.zip
cd ..
