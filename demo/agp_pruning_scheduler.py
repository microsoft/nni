...
from sparsity_generator import NaiveSparsityGenerrator


class AGPPruningScheduler(PruningScheduler):
    def __init__(self, model: Module, config_list: List[Dict], iteration_num: Optional[int]):
        sparsity_generator = NaiveSparsityGenerrator(config_list, iteration_num, 'agp')
        super().__init__(model, config_list, iteration_num=iteration_num, sparsity_generator=sparsity_generator)
...
if __name__ == '__main__':
    ...
    scheduler = AGPPruningScheduler(model, config_list, 10)
    scheduler.set_pruner(L1FilterPruner)
    scheduler.compress(finetuner=finetuner, finetune_optimizer_gen=get_optimizer, finetune_dataloader=train_loader, finetune_epochs=1,
                       speed_up=True, dummy_input=dummy_input)
