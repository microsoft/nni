...
jupyter_lab_version = jupyterlab.__version__.split('.')[0]

def check_jupyter_lab_version():
    environ_version = os.environ.get('JUPYTER_LAB_VERSION')

    if environ_version:
        if jupyterlab.__version__.split('.')[0] != environ_version.split('.')[0]:
            sys.exit(f'ERROR: To build a jupyter lab extension, run "JUPYTER_LAB_VERSION={jupyterlab.__version__}", current: {environ_version} ')
    elif jupyterlab.__version__.split('.')[0] != '3':
            sys.exit(f'ERROR: To build a jupyter lab extension, run "JUPYTER_LAB_VERSION={jupyterlab.__version__}" first for nondefault version(3.x)')
