...
def sst_get_trainable_data(phrases, max_input_length):
    texts, labels = [], []
    ...
    return SSTDataset(texts, labels)
...
def read_data_sst(data_path='/data/v-yugzh/textnas', max_input_length=64, min_count=1, train_with_valid=False,
    ...
    dataset_train = sst_get_trainable_data(train_phrases, max_input_length)
    logger.info("Loaded %d training samples.", len(dataset_train))
    dataset_valid = sst_get_trainable_data(valid_phrases, max_input_length)
    logger.info("Loaded %d validation samples.", len(dataset_valid))
    dataset_test = sst_get_trainable_data(test_phrases, max_input_length)
