...
from ..common import graph_to_module_instance
from ..searchspace import darts
...
def main(model):
    ...
    model_parameters = [p for name, p in model.named_parameters() if not name.endswith('._arch_weight')]
    arch_parameters = [p for name, p in model.named_parameters() if name.endswith('._arch_weight')]
...
def example():
    model_graph = darts()
    model = graph_to_module_instance(model_graph, ['examples.allstars.searchspace.nasnet'])
    main(model)
