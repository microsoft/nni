...
import torch
from torch.utils.tensorboard._pytorch_graph import GraphPy, NodePy, NodePyIO, NodePyOP
...
class TorchModuleGraph(TorchGraph):
                    ...
                    if predecessor_node.kind() == 'prim::ListConstruct':
                        ...
                        if args[0] == None:
                            if curr_node.kind() not in ['aten::mean', 'aten::avg_pool2d', 'aten::ones_like', 'aten::to', 'aten::softmax', 'aten::sum', 'aten::arange']:
                                _logger.debug('zql: %s', curr_node)
                            assert curr_node.kind() in ['aten::mean', 'aten::avg_pool2d', 'aten::ones_like',
                                                        'aten::to', 'aten::softmax', 'aten::sum', 'aten::arange']
                        ...
                        raise RuntimeError(
                            'Node type {} is not correctly handled during retrieving input arguments'.format(predecessor_node.kind()))
                    else:
    ...
    def _expand_module_node(self, node, node_name, unique_name, op_type, nodes,
        ...
        nodepy = NodePyGroup(node_name, unique_name, module_type, op_type,
                             node_group, inputs=list(set(inputs)), outputs=outputs)
        return nodepy
