...
from .pytorch_graph._graph_utils import NodePyGroup, TorchModuleGraph
...
def module_graph_to_ir(base_model, module_graph):
    ...
    for name, module in base_model.named_modules():
        logger.debug('module name: %s', name)
        ...
        for output in node_pygroup.outputs:
            if output in module_graph.input_to_node:
                ...
                if output not in outputs:
                    logger.error('zql output: %s', output)
    ...
    logger.debug('graph json: %s', graph_json)
...
def hack_for_textnas(module_graph, root_tnode):
    ...
    for i in range(24):
        ...
        # create one node
        from .pytorch_graph._graph_utils import NodePyGroup
        subgraph = NodePyGroup(name, name, 'module', 'WrapperOpTN', [], subgraph_inputs, subgraph_outputs)
        new_nodes.append(subgraph)
    # remove nodes and add new nodes, then rebuild index of nodes
    new_node_set = []
    for unique_name, node_pygroup in module_graph.name_to_node.items():
...
def collapse_node(module_graph, root_tnode, node_groups):
    # node_groups is a mapping from key to node type
    to_remove_nodes = {}
    new_nodes = []
    for cluster_name, cluster_type in node_groups.items():
        # get all nodes under this scope name
        subgraph_nodes = {}
        for unique_name, node_pygroup in module_graph.name_to_node.items():
            if node_pygroup.name == cluster_name or node_pygroup.name.startswith(cluster_name + '.'):
                subgraph_nodes[unique_name] = node_pygroup
                to_remove_nodes[unique_name] = node_pygroup
        # get predecessor or successor nodes
        subgraph_inputs = set()  # should be set, as there might be several same inputs
            ...
            for _unique_name in nodes:
                if _unique_name not in subgraph_nodes:
                    if len(module_graph.name_to_node[_unique_name].inputs) == 1:
                        ...
                        subgraph_outputs |= d
        # create one node
        subgraph = NodePyGroup(cluster_name, cluster_name, 'module', cluster_type, [], subgraph_inputs, subgraph_outputs)
        new_nodes.append(subgraph)
    # remove nodes and add new nodes, then rebuild index of nodes
    new_node_set = []
    for unique_name, node_pygroup in module_graph.name_to_node.items():
        if unique_name not in to_remove_nodes:
            new_node_set.append(node_pygroup)
    new_node_set.extend(new_nodes)
    module_graph.name_to_node, module_graph.input_to_node, module_graph.output_to_node = \
        module_graph._build_index(new_node_set)


def gen_pytorch_graph(base_model, dummy_input=None, collapsed_nodes=None):
    ...
    if collapsed_nodes is not None:
        if collapsed_nodes == 'spos':
            hack_for_spos(module_graph, root_tnode)
        elif collapsed_nodes == 'textnas':
            hack_for_textnas(module_graph, root_tnode)
        else:
            collapse_node(module_graph, root_tnode, collapsed_nodes)
