...
def experiment_config(field=None) -> 'Any':
    try:
        global _experiment_config
        if _experiment_config is None:
            _experiment_config = yaml.load(open('experiment.yaml'))
        return _experiment_config if field is None else _experiment_config.get(field)
    except:
        return None
