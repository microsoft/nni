from ..graph_v2 import BadGraph, Cell, Edge, Graph, Node
...
def _sort_incoming_edges(node: Node) -> List[Edge]:
    ...
    if all(isinstance(edge.tail_idx, int) for edge in edges):
        ...
        if [edge.tail_idx for edge in edges] == list(range(len(edges))):
            return edges
    raise BadGraph(node.graph, 'Node {} has bad inputs'.format(node.name))
...
def graph_to_tensorflow_model(graph_name: str, graph: Graph) -> str:
    ...
    for node in nodes:
        ...
        if isinstance(node, Cell) and node.template_name == 'WannNet':  # FIXME: temporary hack
