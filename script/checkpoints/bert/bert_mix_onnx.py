...
# the mask for the fp32 values
fp32_mask = torch.load('checkpoints/mixed/fp32_mask.pth')
for name in fp32_mask:
    for key in fp32_mask[name]:
        _pos = fp32_mask[name][key]!=0
        propagated_mask[name][key].data[_pos] = 1
export_tesa(norm_model.cpu(), data, 'artifact_bert_mixed_onnx_with_tesa', propagated_mask)
torch.save(fp32_mask , 'artifact_bert_mixed_onnx_with_tesa/fp32_mask.pth')