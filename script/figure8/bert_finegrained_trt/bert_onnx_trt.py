...
class BertCalibrator(trt.IInt8Calibrator):
    ...
    def get_batch(self, names):
        ...
        cuda.memcpy_htod(self.device_input[1], self.data[1].ravel())
        cuda.memcpy_htod(self.device_input[2], self.data[2].ravel())
...
def inference(context, test_data):
    ...
    result = []
    inputs[0].host = test_data[0]
    inputs[1].host = test_data[1]
    inputs[2].host = test_data[2]
...
def build_engine(model_file, calib, batch_size=32):
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network(common.EXPLICIT_BATCH) as network, trt.OnnxParser(network, TRT_LOGGER) as parser, builder.create_builder_config() as trt_config:
        ...
        for i in range(network.num_layers):
            ...
            #if layer.name[0:5] != "Shape":
            if layer.name[0:4] in support_op:
...
onnx_path = "../../checkpoints/bert/artifact_bert_finegrained_no_propagation_onnx_with_tesa/model_no_tesa.onnx"
dummy_input = (torch.rand(32, 128).numpy(), torch.rand(32, 128).numpy(), torch.rand(32, 128).numpy())
batch_size = 32
calib = BertCalibrator(dummy_input)
engine = build_engine(onnx_path, calib, batch_size)
