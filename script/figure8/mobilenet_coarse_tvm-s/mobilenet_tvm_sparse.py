...
parser.add_argument('--model_path', type=str, default='../../checkpoints/mobilenet/artifact_mobilenet_coarsegrained_no_propagation_onnx_with_tesa/model_no_tesa.onnx', help='The file name of the frozen graph.')
...
log_file = "mobilenet_coarse_autotvm.log"
...
m.set_input("input.1", data_tvm)
#m.set_input(0, data_tvm)
m.set_input(**{k:tvm.nd.array(v, ctx) for k, v in params.items()})
print("Begin to evaluate")
m.run()
e = m.module.time_evaluator("run", ctx, number=100, repeat=2)
...
# run_relay_graph(mod, params, shape_list, target, ctx)
