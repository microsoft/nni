...
__global__ void BLOCK_SPARSE_MATMUL(float* A, float* W_val, int* W_row, int* W_col, float* C, float *bias, int M, int K, int N){
    ...
    int A_THREAD_PER_ROW = BLOCK_SIZE_K / 4;    // 8
    int B_THREAD_PER_ROW = BLOCK_SIZE_N / 4;    // 8

    int bszy = BLOCK_SIZE_M / THREAD_SIZE_M;    // 4
    int bszx = BLOCK_SIZE_N / THREAD_SIZE_N;    // 4

    int THREADS_PER_BLOCK = bszy * bszx;    // 16

    int A_TILE_ROW_STRIDE = THREADS_PER_BLOCK / A_THREAD_PER_ROW;   // 2
    int B_TILE_ROW_STRIDE = THREADS_PER_BLOCK / B_THREAD_PER_ROW;   // 2
    ...
    const int vBLOCK_SIZE_M = BLOCK_SIZE_M / THREAD_SIZE_M;     // 4
    const int vBLOCK_SIZE_N = BLOCK_SIZE_N / THREAD_SIZE_N;     // 4
...
int matrixMultiply(int M, int N, int K){
    ...
    */
    ...
    for(int i = 0; i < N; i++){
        h_bias[i] = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
    }
