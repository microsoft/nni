...
* **population_size** (*int value (should > 0), optional, default = 20*) - the initial size of the population (trial num) in evolution tuner. 建议 `population_size` 比 `concurrency` 取值更大，这样用户能充分利用算法（至少要等于 `concurrency`，否则 Tuner 在生成第一代参数的时候就会失败）。
...
**Please note that SMAC doesn't support running on Windows currently. 原因参考：[github issue](https://github.com/automl/SMAC3/issues/483).**
