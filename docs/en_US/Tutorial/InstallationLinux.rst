...
^^^^^^^^^^^^^^^^^^^^^^^^^
  ...
  You can also install NNI in a docker image. Please follow the instructions `here <https://github.com/Microsoft/nni/tree/v1.9/deployment/docker/README>`__ to build an NNI docker image. The NNI docker image can also be retrieved from Docker Hub through the command ``docker pull msranni/nni:latest``.
