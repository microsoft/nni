...
|[__BOHB Tuner__](#BOHBTuner)|BOHB relies on HB(Hyperband) to determine the number of configurations to evaluate with which budget, but it replaces the random selection of configurations at the beginning of each HB iteration by a model-based search(Byesian Optimization). [Reference Paper](https://arxiv.org/abs/1807.01774)|
