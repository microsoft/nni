...
pytorch
```python 
model = nni.algorithms.compression.pytorch.NaiveQuantizer(model).compress()
```
...
PyTorch code
```python
from nni.algorithms.compression.pytorch import QAT_Quantizer
model = Mnist()
...
PyTorch code
```python
from nni.algorithms.compression.pytorch import DoReFaQuantizer
config_list = [{ 
    'quant_types': ['weight'],
    'quant_bits': 8, 
    'op_types': 'default' 
...
PyTorch code
```python
from nni.algorithms.compression.pytorch import BNNQuantizer
