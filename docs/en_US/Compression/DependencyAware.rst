...
As `dependency analysis utils <./CompressionUtils.md>`_ shows, if the output channels of two convolutional layers(conv1, conv2) are added together, then these two conv layers have channel dependency with each other(more details please see `Compression Utils <./CompressionUtils.md>`_\ ). Take the following figure as an example.
