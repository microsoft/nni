...
Since some trials need to load other trial's checkpoint, users should provide a directory (i.e., ``all_checkpoint_dir``\ ) which is accessible by every trial. It is easy for local mode, users could directly use the default directory or specify any directory on the local machine. For other training services, users should follow `the document of those training services <../TrainingService/Overview>`__ to provide a directory in a shared storage, such as NFS, Azure storage.
...
The complete example code can be found `here <https://github.com/microsoft/nni/tree/v1.9/examples/trials/mnist-pbt-tuner-pytorch>`__.
