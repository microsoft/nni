...
**classic_mode**: this mode is described [above](#ProgInterface), in this mode, each subgraph runs as a subgraph.
**enas_mode**: (currently only supported on tensorflow)
**oneshot_mode**: (currently only supported on tensorflow)
**darts_mode**: (not supported yet)
