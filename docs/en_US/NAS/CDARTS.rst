...
In implementation of ``CdartsTrainer``\ , it first instantiates two models and two mutators (one for each). The first model is the so-called "search network", which is mutated with a ``RegularizedDartsMutator`` -- a mutator with subtle differences with ``DartsMutator``. The second model is the "evaluation network", which is mutated with a discrete mutator that leverages the previous search network mutator, to sample a single path each time. Trainers train models and mutators alternatively. Users can refer to `references <#reference>`__ if they are interested in more details on these trainers and mutators.
...
`Example code <https://github.com/microsoft/nni/tree/v1.9/examples/nas/cdarts>`__
