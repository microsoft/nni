...
class RetiariiAdvisor(MsgDispatcherBase):
    ...
    def _validate_placement_constraint(self, placement_constraint):
        ...
        if placement_constraint['type'] == 'GPUNumber' and len(placement_constraint['gpus'] == 1):
            raise ValueError('placement_constraint.gpus must be a list 1 number (single host).')
