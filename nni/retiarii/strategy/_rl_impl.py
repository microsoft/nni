...
class ModelEvaluationEnv(gym.Env):
    ...
    def observation_space(self):
        return spaces.Dict(
            {
                "action_history": spaces.MultiDiscrete(
                    [self.action_dim] * self.num_steps
                ),
...
class Preprocessor(nn.Module):
    ...
    def forward(self, obs):
        ...
        return feature[
            torch.arange(len(feature), device=feature.device),
            obs["cur_step"].long() + 1,
        ]
...
class Actor(nn.Module):
    ...
    def forward(self, obs, **kwargs):
        ...
        mask = torch.arange(self.action_dim).expand(len(out), self.action_dim) >= obs[
            "action_dim"
        ].unsqueeze(1)
