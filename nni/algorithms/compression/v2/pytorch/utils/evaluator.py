...
class LightningEvaluator(Evaluator):
    def __init__(self, trainer: pl.Trainer, data_module: pl.LightningDataModule, dummy_input: Any | None = None):
        ...
        self._optimizer_helpers: List[OptimizerConstructHelper] | None = None
        self._lr_scheduler_helpers: List[LRSchedulerConstructHelper] | None = None
        # record i-th lr_scheduler scheduling j-th optimizer lr
        self._lrs_opt_map: Dict[int, int] | None = None
        # record `LightningModule.configure_optimizers` 6-th option returned dict information
        self._configure_optimizers_returned_dicts: List[Dict] | None = None
    ...
    def init_optimizer_helpers(self, pure_model: pl.LightningModule):
        if self._optimizer_helpers is None:
            ...
            # 6. Tuple of dictionaries as described above, with an optional "frequency" key.
                ...
                err_msg = f'Got an wrong returned value type of `LightningModule.configure_optimizers`: list or tuple of {type(optimizers_lr_schedulers[0]).__name__}'
...
class LegacyEvaluator(Evaluator):
    def __init__(self, trainer: _TRAINER, optimizers: Optimizer | List[Optimizer], criterion: _CRITERION,
        ...
        self.trainer = trainer
        self._traced_optimizers = optimizers if isinstance(optimizers, (list, tuple)) else [optimizers]
        assert all(isinstance(optimizer, Optimizer) and is_traceable(optimizer) for optimizer in self._traced_optimizers)
    ...
    def init_optimizer_helpers(self, pure_model: Module):
        if not self._optimizer_helpers:
