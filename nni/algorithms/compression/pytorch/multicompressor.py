...
class MultiCompressor:
    ...
    def compress(self, model_path: str, mask_path: str = None, calibration_path: str = None, onnx_path: str = None,
        ...
        if len(self.pruner_config_list) > 0:
            mask_dict = self.export_pruned_model(model_path, mask_path, onnx_path, input_shape, self.device)
            dummy_input = torch.randn(input_shape).to(self.device)
            model_sp = ModelSpeedup(self.bound_model, dummy_input, mask_dict)
        ...
        if self.trainer and len(self.quantizer_config_list) > 0:
            self.trainer.finetune(self.bound_model, self.optimizer, self)
            calibration_config = self.export_quantized_model(model_path, calibration_path, onnx_path, input_shape, self.device)
        return self.bound_model

    def export_pruned_model(self, model_path: str, mask_path: str = None, onnx_path: str = None,
    ...
    def export_quantized_model(self, model_path: str, calibration_path: str = None, onnx_path: str = None,
                               input_shape: Optional[Union[List, Tuple]] = None, device: torch.device = None):
        assert model_path is not None, 'model_path must be specified'
        calibration_config = {}
