...
class Trial:
    ...
    def run(self):
        ...
        trial_dir = os.path.realpath(os.path.join(os.curdir, f"trial_{self.id}"))
        os.makedirs(trial_dir, exist_ok=True)
        self.trial_dir = trial_dir

        environ = os.environ.copy()
        environ['NNI_TRIAL_SEQ_ID'] = str(self.data["sequenceId"])
        environ['NNI_OUTPUT_DIR'] = trial_dir
        environ['NNI_SYS_DIR'] = trial_dir

        # prepare parameters
        nni_log(LogType.Info, '%s: saving parameter %s' % (self.name, self.data["parameter"]))
        parameter_file_name = os.path.join(trial_dir, "parameter.cfg")
        with open(parameter_file_name, "w") as parameter_file:
            parameter_file.write(self.data["parameter"])

        # FIXME: seems this part is for distributed trial
        ...
        self.process = Popen(trial_command, shell=True, stdout=self.log_pipe_stdout,
                             stderr=self.log_pipe_stdout, cwd=os.path.join(os.curdir, "code"), env=dict(environ))
    ...
    # FIXME: command_data may not has field parameters!!!
