...
class NniManagerArgs:
    ...
    def factory_create(exp_id, config, port, debug):
        args = NniManagerArgs()
    ...
    def factory_resume(exp_id, config, port, debug):
        args = NniManagerArgs()
        args.experiment_id = exp_id
        args._load_config(config)
        args.port = port
    ...
    def factory_view(exp_id, config, port, debug):
        args = NniManagerArgs()
        args.experiment_id = exp_id
        args._load_config(config)
        args.port = port
    ...
    def _load_config(self, config):
        ...
        if self.mode == 'openpai':
            self.mode = 'pai'
        self.log_dir = config.experiment_working_directory
        self.log_level = config.log_level

def start_nni_manager(args: NniManagerArgs) -> Popen:
    _ensure_port_idle(args.port)
    _ensure_port_idle(args.port + 1, f'NNI requires an additional port')
    ...
    for field in fields(NniManagerArgs):
