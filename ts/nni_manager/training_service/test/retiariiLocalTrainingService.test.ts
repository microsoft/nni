...
import { RetiariiLocalTrainingService } from '../retiarii_local/retiariiLocalTrainingService';
...
describe('Unit Test for RetiariiLocalTrainingService', () => {
    ...
    it('Read metrics, Add listener, and remove listener', async () => {
        const localTrainingService = new RetiariiLocalTrainingService(config2);
        localTrainingService.run();
    ...
    it('Submit job with device placement constraint', async () => {
        ...
        // submit job asking for GPU-1 and GPU-2 on Node-0
        const form: TrialJobApplicationForm = {
            sequenceId: 0,
            hyperParameters: {
                value: 'mock hyperparameters',
        ...
        // chai.expect(resource).to.deep.equals({gpuIndices:[1,2]});
