...
#define CHUNK_COPY_LINE_LANES (CHUNK_LINE_BYTES / sizeof(int4))
...
#pragma unroll
    ...
    for(int tile_k_idx = 0; tile_k_idx < K_GLOBAL; tile_k_idx += CHUNK_K * K){
      ...
      if(warpId < (WARPS_PER_BLOCK / 2)){
        ...
        lane_ptr = (int4 *)(warp_ptr + tile_k_idx +
                          (laneId / CHUNK_COPY_LINE_LANES) * K_GLOBAL) +
                          (laneId % CHUNK_COPY_LINE_LANES);
      }else{
        const uint8_t *warp_ptr = &B[block_tile_j * N * K_GLOBAL] +
          (warpId % (WARPS_PER_BLOCK / 2)) * CHUNK_COPY_LINES_PER_WARP * K_GLOBAL;
        lane_ptr = (int4 *)(warp_ptr + tile_k_idx + 
                          (laneId / CHUNK_COPY_LINE_LANES) * K_GLOBAL) +
                          (laneId % CHUNK_COPY_LINE_LANES);
      ...
      for (int i = 0; i < iter_index; i++) {
        ...
        *((int4 *)&shmem[shmem_idx][0] + (laneId % CHUNK_COPY_LINE_LANES)) =
        ...
        lane_ptr = (int4 *)((uint8_t *)lane_ptr + K_GLOBAL * (WARPS_PER_BLOCK / 2) * CHUNK_COPY_LINES_PER_WARP);
...
void HostComputation(uint8_t* A, uint8_t* W, uint8_t* D, int *bias, int alpha, int integer){
...
int matrixMultiply(){
  ...
  int size_B = N_GLOBAL * K_GLOBAL;
  int size_C = M_GLOBAL * N_GLOBAL;

  int mem_size_A = sizeof(uint8_t) * size_A;
  int mem_size_B = sizeof(uint8_t) * size_B;
  ...
  uint8_t* h_B = (uint8_t*)malloc(mem_size_B);
  ...
  for(int i = 0; i < M_GLOBAL; i++){
      for(int j = 0; j < K_GLOBAL; j += 1){
        h_A[i * K_GLOBAL + j] = rand()%5;
      }
  }

  for(int i = 0; i < N_GLOBAL; i += 1){
      for(int j = 0; j < K_GLOBAL; j += 1){
        h_B[i * K_GLOBAL + j] = rand()%5;
      }
  }
  ...
  for(int run = 0; run < nIter; run++){
    checkKernelErrors(
        (compute_gemm_imma<<<block_num, THREADS_PER_BLOCK>>>(d_A, d_B, d_D, d_bias, alpha, integer)));
  }
  ...
  // HostComputation_sparse(h_A, h_row, h_col, h_val, h_C, BLOCK_SIZE_N, BLOCK_SIZE_K, alpha, integer);

  HostComputation(h_A, h_B, h_D, h_bias, alpha, integer);
  ...
  for(int i = 0; i < M_GLOBAL * N_GLOBAL; i++){
      double abs_err = fabs(h_D[i] - h_result[i]);
      double dot_length = M;
      double abs_val = fabs(h_D[i]);
      double rel_err = abs_err / abs_val / dot_length;
      if (rel_err > eps) {
          printf("Error! Matrix[%05d]=%d, ref=%d error term is > %E\n",
                  i, h_result[i], h_D[i], eps);
          correct = false;
          break;
      }
  }
