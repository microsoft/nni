...
class PruningToolsTestCase(unittest.TestCase):
    def test_data_collector(self):
        ...
        # Test TargetDataCollector
        data_collector = TargetDataCollector(pruner)
        ...
        # Test EvaluatorBasedTargetDataCollector
        def opt_after():
        ...
        data_collector = EvaluatorBasedTargetDataCollector(pruner, trainer, optimizer_helper, criterion, 1, opt_after_tasks=[opt_after])
        data = data_collector.collect()
        assert all(torch.equal(get_module_by_name(model, module_name)[1].weight.data, data[module_name]) for module_name in ['conv1', 'conv2'])
        assert all(t.numel() == (t == 1).type_as(t).sum().item() for t in data.values())

        # Test EvaluatorBasedHookDataCollector
        ...
        optimizer_helper = OptimizerConstructHelper.from_trace(model, get_optimizer(model))
        data_collector = EvaluatorBasedHookDataCollector(pruner, trainer, optimizer_helper, criterion, 2, collector_infos=[collector_info])
        data = data_collector.collect()
