...
class Quantizer(Compressor):
    ...
    def _instrument_layer(self, layer, config):
        ...
        assert isinstance(config["quant_bits"], int) or isinstance(config["quant_bits"], dict), \
            'quant_bits must be dict type or int type'

        if isinstance(config["quant_bits"], dict):
            for quant_type in config["quant_types"]:
                assert quant_type in config["quant_bits"], \
                    'bits length for %s must be specified in quant_bits dict' % quant_type
